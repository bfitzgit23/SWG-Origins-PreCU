...
const configFile = require('os').homedir() + '/swg-origins.json';
var config = {folder: 'C:\\SWG-Origins'};
...
websiteBtn.addEventListener('click', event => shell.openExternal("https://www.swg-origins.com"));
discordBtn.addEventListener('click', event => shell.openExternal("https://discord.gg/V8UP5rgREz"));
...
if (fs.existsSync(path.join(config.folder, '07_Nova2_JediCreationR4.tre'))) {
    disableAll();
    resetProgress();
    install.install(config.folder, config.folder, config.mods);
} else {
